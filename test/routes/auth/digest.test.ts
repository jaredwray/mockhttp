...
describe('GET /digest-auth', () => {
	...
	it.skip('authenticates successfully with valid digest', async () => {
		...
		try {
			...
			const ha1 = crypto.createHash('md5').update(`testuser:${realm}:testpass`).digest('hex');
			const ha2 = crypto.createHash('md5').update(`GET:${uri}`).digest('hex');
			...
			const body = JSON.parse(String(response.body)) as {authenticated: boolean; user: string};
	...
	it('401 when username mismatch on algorithm endpoint', async () => {
		...
		const response = await fastify.inject({
			...
			headers: {authorization: 'Digest username="wronguser", response="test"'},
		});
		expect(response.statusCode).toBe(401);
		expect(response.headers['www-authenticate']).toMatch(/algorithm=MD5/);
	});
	...
	it('401 when no qop on algorithm endpoint with wrong response', async () => {
		...
		const wrongResponse = crypto.createHash('sha256').update(`${ha1}:${nonce}:${ha2}WRONG`).digest('hex');
	...
	it('401 when response is wrong on algorithm endpoint', async () => {
		...
		const response = await fastify.inject({
			...
			headers: {authorization: 'Digest username="user", response="wrongresponse"'},
		});
		expect(response.statusCode).toBe(401);
	...
	it.skip('authenticates successfully on algorithm endpoint', async () => {
		...
		try {
			...
			const ha1 = crypto.createHash('sha256').update(`testuser:${realm}:testpass`).digest('hex');
			const ha2 = crypto.createHash('sha256').update(`GET:${uri}`).digest('hex');
			const responseHash = crypto.createHash('sha256')
				.update(`${ha1}:${nonce}:${nc}:${cnonce}:${qop}:${ha2}`)
				.digest('hex');

			const validAuthHeader = `Digest username="testuser", realm="${realm}", nonce="${nonce}", uri="${uri}", `
