...
describe('GET /hidden-basic-auth/:user/:passwd', () => {
	it('mirrors /basic-auth behavior', async () => {
		...
		const response401 = await fastify.inject({method: 'GET', url: '/hidden-basic-auth/user/pass'});
		expect(response401.statusCode).toBe(401);

		const response200 = await fastify.inject({method: 'GET', url: '/hidden-basic-auth/user/pass', headers: {authorization: makeBasic('user', 'pass')}});
